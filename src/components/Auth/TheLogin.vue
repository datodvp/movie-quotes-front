    ...
    <PopUpCard>
        <div class="flex flex-col items-center justify-center w-full">
            ...
            <Form @submit="authorize" class="w-full px-4 md:px-0">
                <TextInput :label="$t('auth.email_or_username')" name="login"
                    :placeholder="$t('auth.email_or_username_placeholder')" v-model="login" :rules="{ required: true }" />
                ...
                <div class="flex justify-between mt-4">
                    ...
                    <RouterLink :to="{ name: 'forgotPassword' }"
                        class="text-[#0D6EFD] underline flex justify-center items-center">{{ $t("auth.forgot_password") }}
                    </RouterLink>
                </div>
                ...
                <SecondaryButton @click="loginGoogle" class="mt-4">
                    ...
                    <button type="button">
                        <IconGoogle class="mr-2" />{{ $t("auth.sign_in_google") }}
                    </button>
                </SecondaryButton>

                <div class="flex justify-center gap-1 mt-8">
                    <p>{{ $t("auth.dont_have_account") }}</p>
                    <RouterLink to="/sign-up" class="text-[#0D6EFD] underline">{{ $t("auth.sign_up") }}</RouterLink>
                </div>
